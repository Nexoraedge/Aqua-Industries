            ...
            {faqs.map((faq, idx) => (
                <div key={idx} className="border border-slate-200 bg-white shadow-sm overflow-hidden rounded-none">
                    <button
                        ...
                        className="w-full text-left p-6 flex justify-between items-center hover:bg-slate-50 transition-colors group"
                    >
...
const AdvancedCoverageCalculator = () => {
    ...
    // Industry Standard Formula for Tile Adhesive:
    // Density is approx 1.6 kg / (m² * mm)
    // Area in Sq.Ft to Sq.M = Area / 10.764
    // Total Kg = (SqM Area) * (Thickness in mm) * 1.6
...
export default function ProductDetailPage({ params }: { params: Promise<{ category: string, productId: string }> }) {
    const resolvedParams = use(params);
    const product = (detailedProducts as Record<string, any>)[resolvedParams.productId] || (detailedProducts as Record<string, any>)["ultima-bond-1t"];

    const isAdhesive = product?.categorySlug === 'ultima-bond-tile-adhesives';
    const isEpoxy = product?.categorySlug === 'gripoxy-epoxy-grouts';
    const isPolymer = product?.categorySlug === 'ultima-v-two-grout';
        ...
        <div ref={container} className="bg-[#fcfbf9] min-h-screen selection:bg-brand-900 selection:text-white pb-32 pt-28 font-sans text-slate-900">
            <div className="px-6 sm:px-12 max-w-[1400px] mx-auto mt-12">
                ...
                <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-brand-950/40 mb-16">
                    ...
                    <Link href={`/products/${resolvedParams.category}`} className="hover:text-brand-600 transition-colors">{product.categoryName}</Link>
                    <span className="text-brand-950/20">/</span>
                ...
                {isAdhesive && (
                    <div className="scroll-section mb-16 relative pt-16 border-t border-slate-200">
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8 bg-brand-950 p-12 lg:p-16 text-white shadow-2xl relative overflow-hidden rounded-none">
                            <div className="absolute inset-0 bg-[url('/assest/textures/concrete_texture.jpg')] opacity-10 mix-blend-overlay pointer-events-none" />
                ...
                <div className="scroll-section pt-32 border-t border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {/* Dynamic Related Product Cards */}
                        {relatedProducts.map((relProd: any) => (
                            <Link href={`/products/${relProd.categorySlug}/${relProd.id}`} key={relProd.id} className="bg-white border border-slate-200 p-8 hover:border-brand-400 hover:shadow-xl transition-all duration-300 group flex flex-col justify-between rounded-none">
                                <div className="mb-10">
